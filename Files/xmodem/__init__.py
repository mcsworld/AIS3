        ...
        while True:
            ...
            while True:
                ...
                if packet_size == 128:
                    ...
                    p.append(STX)
                p.append(chr(sequence))
                p.append(chr(0xff - sequence))
                p.append(data)
                if crc_mode:
                    p.append(chr(crc >> 8))
                    p.append(chr(crc & 0xff))
                else:
                    p.append(chr(crc))
                self.putc(''.join(p))
                ...
                if char == ACK:
                    ...
                    break
                if char == NAK:
                    print >> sys.stderr, "warning: get NAK and retry"
                    error_count += 1
                    if callable(callback):
                        callback(total_packets, success_count, error_count)
                    if error_count >= retry:
                ...
                # protocol error
                print >> sys.stderr, "error: code1=%d, code2=%d" % (ord(char), self.getc(1, timeout))
                self.abort(timeout=timeout)
